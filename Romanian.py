...
ro_voxpopuli_dataset = load_dataset("facebook/voxpopuli", "ro", split=['train', 'test'])

dataset = ro_voxpopuli_dataset.remove_columns(['audio_id', 'language', 'raw_text', 'gender', 'speaker_id', 'is_gold_transcript', 'accent'])
...
tokenizer = WhisperTokenizer.from_pretrained(model_checkpoint, language="Romanian", task="transcribe")
processor = WhisperProcessor.from_pretrained(model_checkpoint, language="Romanian", task="transcribe")
