...
hu_voxpopuli_dataset = load_dataset("facebook/voxpopuli", "hu", split=['train', 'test'])

dataset = hu_voxpopuli_dataset.remove_columns(['audio_id', 'language', 'raw_text', 'gender', 'speaker_id', 'is_gold_transcript', 'accent'])
...
tokenizer = WhisperTokenizer.from_pretrained(model_checkpoint, language="Hungarian", task="transcribe")
processor = WhisperProcessor.from_pretrained(model_checkpoint, language="Hungarian", task="transcribe")
