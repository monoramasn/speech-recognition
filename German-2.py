...
voxpopuli_dataset = load_dataset("facebook/voxpopuli", "de", split=["train", "test"])
# Extract train and test splits
train_data = voxpopuli_dataset[0]
test_data = voxpopuli_dataset[1]

#dataset_lt = voxpopuli_dataset.remove_columns(['audio_id', 'language', 'raw_text', 'gender', 'speaker_id', 'is_gold_transcript', 'accent'])
train_data = train_data.remove_columns(['audio_id', 'language', 'raw_text', 'speaker_id', 'is_gold_transcript', 'accent'])
test_data = test_data.remove_columns(['audio_id', 'language', 'raw_text', 'speaker_id', 'is_gold_transcript', 'accent'])

# Define the percentage of data you want to download
percentage_to_download = 0.6  # Example: 50%

# Calculate the number of samples to download
total_train_samples = len(train_data)
total_test_samples = len(test_data)
num_train_samples_to_download = int(percentage_to_download * total_train_samples)
num_test_samples_to_download = int(percentage_to_download * total_test_samples)

# Download the desired percentage of data
train_data = train_data.shuffle().select(range(num_train_samples_to_download))
test_data = test_data.shuffle().select(range(num_test_samples_to_download))
...
train_data_map = train_data.map(prepare_dataset, batch_size=8)
test_data_map = test_data.map(prepare_dataset, batch_size=8)
